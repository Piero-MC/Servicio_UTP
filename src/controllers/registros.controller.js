...
export const createRegistros = async (req,res)=> {
    try {
      const {nombre,dni,ingresante,motivo,sede,fecha,estado} =req.body
      const [rows]= await pool.query(`INSERT INTO registro (
            registro_nombre,
            registro_dni,
            registro_tipo_ingresante,
            registro_motivo,
            registro_sede,
            registro_ingreso_fecha,
            estado
        )
        VALUES (
            ?,
            ?,
            (SELECT id_ingresante FROM ingresante WHERE ingresante_detalle = ?),
            (SELECT id_motivo     FROM motivo     WHERE motivo_detalle     = ?),
            (SELECT sede_id FROM registro_sede WHERE sede = ?),
            ?,
            ?
        );`, [nombre,dni,ingresante,motivo,sede,fecha,estado])
              res.send({
                  nombre,
                  dni,
                  ingresante,
